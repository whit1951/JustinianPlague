...
beta_p <-qunif(LHS[,1], min=0.01, max=1) #qtriangle(LHS[,1], a=0.42, b=0.48, c=0.45)
...
alpha<- qunif(LHS[,2], min=0.39/N_r0, max=20/N_r0) #500000= K_r or S_r(t=0)
...
#non uniform distributions
LHS<-optimumLHS(n=h, k=length(parameters), maxSweeps=2, eps=.1, verbose=FALSE)
r_r<- qunif(LHS[,1], min=4/365, max=6/365) #500000= K_r or S_r(t=0)
K_r<- qunif(LHS[,2], min=N_r0/2, max=N_r0*2) #500000= K_r or S_r(t=0)
p_r<- qunif(LHS[,3], min=0.4, max=0.9) #500000= K_r or S_r(t=0)
d_r<- qunif(LHS[,4], min=0.1/365, max=0.3/365) #500000= K_r or S_r(t=0)
beta_r <-qtriangle(LHS[,5], a=0.0, b=3.67, c=1.248)
alpha<- qunif(LHS[,6], min=0.39/N_r0, max=20/N_r0) #500000= K_r or S_r(t=0)
...
# K_l <- qtriangle(LHS[,2], a= 10.5, b=67.7, c=15) #b=67.7
# beta_low <- qtriangle(LHS[,3], a=0, b=0.05, c=0.04)
# beta_high <- qtriangle(LHS[,4], a=0, b=1, c=0.39) #b=1
